...
export default function TradeCyclesBar({
  ...
  const selectedCycle = tradeData?.trade_cycles?.find(
    (cycle) => cycle.cycle_id === selectedCycleNumber
  );
  ...
  useEffect(() => {
    ...
    setSelectedCycleNumber(tradeData?.trade_cycles?.at(-1)?.cycle_id);
                ...
                .map((item, index) => (
                  <div
                    className={`flex flex-col gap-3 font-light rounded-lg border p-3 min-w-72 cursor-pointer  ${
                      item?.cycle_id == selectedCycleNumber
                        ? "bg-green-400"
                        : "hover:bg-green-500 hover:bg-opacity-20"
                    }`}
                    onClick={() => setSelectedCycleNumber(item?.cycle_id)}
                  >
                    <span>
                      Cycle ID:{" "}
          ...
          <table className="w-full">
            ...
            <tbody className="text-[12px] sm:text-[16px]">
              <tr className=" border-b border-gray-700 font-extralight">
                ...
                <td className="py-2 px-">Pair</td>
                <td className="py-2 px-">{tradeData?.daily_profit}</td>
                <td className="py-2 px-">5000 USDT</td>
                <td className="py-2 px-">Action</td>
              </tr>
            </tbody>
          </table>
        </div>
              ...
              .map((cycle, indxe) => (
                <span
                  className={`p-3 py-5 rounded-t-lg cursor-pointer  ${
                    cycle?.cycle_id == selectedCycleNumber
                      ? "bg-white "
                      : "text-white hover:bg-white hover:bg-opacity-20"
                  }`}
                  onClick={() => setSelectedCycleNumber(cycle?.cycle_id)}
                >
                  Cycle ID: {cycle?.cycle_id}
                </span>
              ))
          ) : (
            <div>No trade cycles found.</div>
          )}
        </div>
        ...
        <Tabs>
          <TabList>
            {tradeData?.trade_cycles?.map((cycle) => (
              <Tab key={cycle?.cycle_number}>Cycle ID: {cycle.cycle_id}</Tab>
            ))}
          </TabList>
          <TabPanels>
            <TabPanel>
